...
# print(person.df[1]) # False
print(person.df[1,]) # True
...
# person.df[_____, _____]
# person.df[days.postion] # False
person.df[days.postion, ] # True

# 使用grepl()，找出person.df$person.id中有520精神的
# spirit.520 <- grepl(____, ______)
# spirit.520 <- grepl(person.df, person.id) # False
spirit.520 <- grepl("520", person.df$person.id) #True

# 篩選出520家族的成員
# person.df[_____, _____]
# person.df[spirit.520] # False
person.df[spirit.520, ] # True
