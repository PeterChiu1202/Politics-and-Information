...
df <- read.table('test_data.csv', sep = ',', header = T, stringsAsFactors = F) # True
...
# test_data$v1 <- as.factor(test_data$School) # False
# test_data$gender <- as.factor(test_data$Sex) # False
df$School <- as.factor(df$School) # True
df$Sex <- as.factor(df$Sex) # True
...
# df <- cbind(Height, Weight) # False
df <- cbind(df, Height, Weight) # True

# 使用rbind()，新增kobe至df為列(row)資料
# test_data <- rbind(Kobe) # False
df <- rbind(df, Kobe) # True
...
# names(df)[Grade] <- "ToeicGrade" # False
names(df)[4] <- "ToeicGrade" # True
# [4] == 第四欄
...
# df$Bmi <- mapply(df$Height, df$Weight) # False
df$Bmi <- mapply(GetBmi, df$Height, df$Weight) # True
